...
class BiRefNet(nn.Module):
    def __init__(self, bb_pretrained=True):
        ...
        if self.config.official == False:
            ...
            if self.config.out_ref == True:
                self.laplacian = True
        else:
            self.decoder = Decoder_official(channels)
    ...
    def forward_ori(self, x):
        ...
        # if not self.config.official and self.config.out_ref:
        if self.laplacian:
...
class Decoder_official(nn.Module):
    def __init__(self, channels):
        super(Decoder_official, self).__init__()
        self.config = config
        DecoderBlock = eval(self.config.dec_blk)
        LateralBlock = eval(self.config.lat_blk)
